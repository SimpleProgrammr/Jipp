...
enum STOP_REASON { MAX_M, MAX_DEF_PRECISION, NORMAL };
...
int main() {
    ...
    printf("A <= x <= B\nZakres musi znajdowac sie w przedziale -1 < x <= 1\n");
    ...
    clearStdin();
    printf("Podaj ilość podziałów (N > 0): ");
    if (scanf("%Le", &nc) != 1 || nc <= 0 || round(nc) != nc) {
        fprintf(stderr, "Bledne dane wejsciowe\n");
        return 400;
    }
    clearStdin();
    printf("Podaj dokładność wyników (DeltaY > 0): ");
    if (scanf("%Le", &deltaY) != 1 || deltaY <= 0 || isnan(deltaY) || isinf(deltaY)) {
        fprintf(stderr, "Bledne dane wejsciowe\n");
        return 400;
    }
    clearStdin();
    ...
    if (file == NULL) {
        fprintf(stderr, "Blad pliku!\n");
        exit(404);
    }

    printf("Lp.\t\tX\t\t\t\tf_szereg(X)\t\tf_scisle(X)\t\tWyrazy_szeregu\t\tPowod zatrzymania\n");
    fprintf(file, "Lp.\t\tX\t\t\t\tf_szereg(X)\t\tf_scisle(X)\t\tWyrazy_szeregu\t\tPowod zatrzymania\n");
    ...
    for (long long counter = 1; counter <= n; counter++) {
        ...
        switch (stop_res) {
            case MAX_M:
                stop_msg = "Przekroczono ilosc iteracji";
                break;
            case MAX_DEF_PRECISION:
                stop_msg = "Osiegnieto zadana dokladnosc";
                break;
            default:
                stop_msg = "Praca normalna";
                break;
        }

        printf("%lld\t\t%Le\t%Le\t%le\t%le\t\t%s\n", counter, x, f_x, log(1+x), (double) used_M, stop_msg);
        fprintf(file, "%lld\t\t%Le\t%Le\t%le\t%le\t\t%s\n", counter, x, f_x, log(1+x), (double) used_M, stop_msg);
        x += deltaX;
    }
