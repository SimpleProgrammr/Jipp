...
int main() {
    ...
    if (loadedLines + 1 >= linesInTab) {
        char **tmp = realloc(text, (++loadedLines) * sizeof(char *));
        if (text == NULL) {
    ...
    for (int i = loadedLines; i >= 0; i--) {
        if (i == lineTarget + 1) {
            startText[stringLength - 1] = '\0';
            text[i] = realloc(text[i], stringLength * sizeof(char *));
            strcpy(text[i], startText);
            loadedLines++;
            break;
        }
